...
void MAVConnTCPClient::client_connected(size_t server_channel)
{
	...
	// start recv
	//((boost::asio::io_context&)socket.get_executor().context()).post(std::bind(&MAVConnTCPClient::do_recv, shared_from_this()));
	((boost::asio::io_context&)socket.get_executor().context()).post(std::bind(&MAVConnTCPClient::do_recv, shared_from_this()));
}
...
void MAVConnTCPClient::send_bytes(const uint8_t *bytes, size_t length)
	{
		...
		((boost::asio::io_context&)socket.get_executor().context()).post(std::bind(&MAVConnTCPClient::do_send, shared_from_this(), true));
	}
